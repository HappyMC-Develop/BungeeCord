...
public class CaptchaGeneration
{
    ...
    public void generateImages()
    {
        ...
        {
            BungeeCord.getInstance().getLogger().log( Level.INFO, "[BotFilter] 验证码生成中…… [{0}/900]", 900 - ex.getQueue().size() - ex.getActiveCount() );
            try
            {
                Thread.sleep( 1000L );
            } catch ( InterruptedException ex1 )
            {
                BungeeCord.getInstance().getLogger().log( Level.WARNING, "[BotFilter] 验证码生成失败，即将关闭", ex1 );
                System.exit( 0 );
                return;
            }
        }
        ...
        System.gc();
        BungeeCord.getInstance().getLogger().log( Level.INFO, "[BotFilter] 验证码生成成功耗时{0} ms", System.currentTimeMillis() - start );
    }
